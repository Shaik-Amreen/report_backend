...
fs.writeFile(`files/studentdata/ ${a.yearofjoining} - ${a.year} year - ${a.sem} sem - ${a.department}.json`, data, (err,docs) => {
    ...
    fs.readFile(`files/users.json`, 'utf8' , (er, docsa) => {
        if(er)
        {
            ...
            docsa=JSON.parse(docsa);
             ...
             data.map(c=>{
                 ...
                 if(g.length==0){
                        ...
                        fs.readFile(`files/users.json`, 'utf8' , (error, doc) => {
                        ...
                        res.send({message:'user already exists'})
                            ...
                            docsa.push({mail:c.rollnumber,password:bcrypt.hashSync(c.rollnumber,10),year:a.year,department:a.department,
                                sem:a.sem,yearofjoining:a.yearofjoining,token:encrypt(jwt.sign({subject:c.rollnumber},JWTSECRET))})
...
res.send({message:'saved'})
   ...
   exports.savecoques = (req,res,next) =>{
   ...
   exports.saveassignment = (req,res,next) =>{
...
exports.uploadsubjects = (req,res,next) =>{
...
exports.uploadelectives = (req,res,next) =>{
...
exports.fetchroll = (req,res,next) =>{
    let a=req.body
    fs.readFile(`files/studentdata/ ${a.yearofjoining} - ${a.year} year - ${a.sem} sem - ${a.department}.json`, 'utf8' , (err, docs) => {
        if (err) {
         res.send({message:'error'})
    ...
    exports.fetchrollsection = (req,res,next) =>{
        let a=req.body
        fs.readFile(`files/studentdata/ ${a.yearofjoining} - ${a.year} year - ${a.sem} sem - ${a.department}.json`, 'utf8' , (err, docs) => {
            if (err) {
             res.send({message:'error'})
    ...
    exports.uploadmarks = (req,res,next) =>{
    console.log(req.body)
        let a=req.body.name
        fs.readFile(`files/studentdata/ ${a.yearofjoining} - ${a.year} year - ${a.sem} sem - ${a.department}.json`, 'utf8' , (err, docs) => {
            if (err) {
     ...
     // write JSON string to a file
        ...
        exports.uploadlabmarks = (req,res,next) =>{
...
exports.savefeedback= (req,res,next) =>{
   ...
   exports.savestudentfeedback= (req,res,next) =>{
            ...
            fs.readFile(`files/studentdata/ ${a.yearofjoining} - ${a.year} year - ${a.sem} sem - ${a.department}.json`, 'utf8' , (err, docsa) => {
                ...
                if (err || docsa.length==0) {
                   ...
                   fs.readFile(`files/feedback.json`, 'utf8' , (errs, docsas) => {
                    // convert JSON object to string
                    if (err || docsa.length==0 ||docsas==undefined) {
                       return res.send({message:'error'})     
                    }
                    else{
